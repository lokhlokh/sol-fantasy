...
export const rosterErrorMessages: Record<RosterErrorCode, string> = {
  ...
  BUDGET_EXCEEDED: "장부 가격 합계는 50별 이하여야 합니다.",
  SEASON_TEAM_MINIMUM_NOT_MET: "시즌팀 선수는 최소 3명 포함해야 합니다.",
  ROLE_PLAYER_NOT_IN_LINEUP: "캡틴, 부캡틴은 라인업 안에서 지정해야 합니다.",
  ROLE_CONFLICT: "캡틴, 부캡틴은 서로 다른 선수여야 합니다."
};
...
export function validateRoster(lineup: Lineup, players: Player[]) {
  ...
  const budget = calculateLedgerCost(selected);
  ...
  const roleIds = [lineup.captainId, lineup.viceCaptainId];
  ...
  return {
    ...
    errors,
    budget,
    currentMarketValue: calculateCurrentMarketValue(selected),
    currentTeamValue: calculateCurrentTeamValue(selected),
    seasonTeamCount,
    selected
  };
}
